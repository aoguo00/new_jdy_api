...
from core.third_party_config_area.models import DeviceTemplateModel
...
class DevicePointDialog(QDialog):
    ...
    def __init__(self,
        ...
        self.setWindowTitle("第三方设备点表配置 - 批量配置模式")
        self.resize(1200, 800)  # 增大窗口以容纳新的区域
        ...
        # 新增：模板记忆功能
        self.last_selected_template_name = ""
    ...
    def setup_ui(self):
        ...
        self.description_prefix_input.setPlaceholderText("请输入描述信息，支持使用*作为占位符")
    ...
    def template_selected(self, index: int):
        ...
        # 当模板改变时，我们不清空用户已输入的变量名和描述，除非没有有效模板
        # 用户可能希望对不同模板使用相同的变量名和描述
        # 但是，如果之前没有有效模板，或者新选的模板也无效，则清空输入框
        ...
        # 注意：这里不再从模板设置输入框的值，因为模板不再包含前缀信息

        self.update_point_table()
        self.update_preview() # 模板更改会触发预览更新，预览会使用当前输入框的内容
